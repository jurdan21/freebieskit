...
export default function DetailWebDesignInspiration({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function fetchResource() {
      setLoading(true);
      setError(null);
      // Ambil id kategori web-design-inspiration
  ...
  return (
    <div className="max-w-3xl mx-auto py-16 px-4">
      ...
      <a href={resource.download_link} target="_blank" rel="noopener noreferrer" className="inline-block px-6 py-2 rounded-full bg-blue-600 text-white font-medium shadow hover:bg-blue-700 transition">Visit Website</a>
    </div>
  );
} 